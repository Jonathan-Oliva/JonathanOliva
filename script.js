const i18n = {
    en: {
        skipBtn: "[ SKIP ANIMATION - FAST ACCESS ]",
        langBtn: "ES",
        intro: [
            "> INITIALIZING SYSTEM...",
            "> RESOLVING DEPENDENCIES...",
            "> LOADING GRAPHICAL INTERFACE..."
        ],
        accessGranted: ">>> ACCESS GRANTED. LOADING SYSTEM_PROFILE...",
        personalDataTitle: "PERSONAL DATA",
        name: "NAME: JONATHAN OLIVA",
        role: "ROLE: FULLSTACK DEVELOPER & TECH LEAD / CEO @ NIBBLE",
        location: "LOCATION: Villa Mercedes, San Luis, Argentina",
        techTitle: "TECH STACK AND ARCHITECTURE",
        techSubtitle: "> Languages and tools I use in production:",
        backend: "<span class='skill-category'>BACKEND:</span> Python, Node.js, TypeScript, Java",
        frameworks: "<span class='skill-category'>FRAMEWORKS:</span> Django, Express.js",
        infra: "<span class='skill-category'>INFRA & DATA:</span> Docker, PostgreSQL, SQL, RESTful APIs",
        product: "<span class='skill-category'>PRODUCT (UI/UX):</span> Product Conceptualization, Canva, Dev Experience",
        projectsTitle: "FEATURED PROJECTS",
        project1: "1. SPORTS BOOKING SYSTEM (Nibble) - In Development",
        project1Desc1: "> Scalable back-end architecture implemented in Python/Django and Dockerized.",
        project1Desc2: "> Robust concurrency management: implemented temporal atomic locks to prevent overlapping court reservations.",
        project1Demo: "> Live Demo: <a href='https://app.alapelotita.com.ar/' target='_blank' class='project-link'>app.alapelotita.com.ar</a>",
        project2: "2. HAIR SALON BOOKING SYSTEM",
        project2Desc1: "> REST API built with Node.js/Express for appointment self-management.",
        project2Desc2: "> Complex time-slot allocation logic based on business availability.",
        project2Demo: "> Live Demo: <a href='https://cirobarber.com.ar/' target='_blank' class='project-link'>cirobarber.com.ar</a>",
        project3: "3. NIBBLE STARTUP - CORPORATE PORTFOLIO",
        project3Desc1: "> As CEO, leading UI/UX design, branding (logos, aesthetics) and production web implementation.",
        project3Demo: "> Demo: <a href='https://nibble-web-lemon.vercel.app/' target='_blank' class='project-link'>nibble-web-lemon.vercel.app</a>",
        project4: "4. ELECTRONIC VOTING SYSTEM (Auditing / Academic)",
        project4Desc1: "> Backend development focused on vote security and integrity, mentored directly by a Senior Engineer.",
        project4Desc2: "> Production Soft Skills: Technical representative, presenter, and system defender on local radio media.",
        project4Repo: "> Repo: <a href='https://github.com/EnzoL1016/Proyecto-Unvime' target='_blank' class='project-link'>github.com/EnzoL1016/Proyecto-Unvime</a>",
        contactTitle: "CONTACT",
        emailLink: "EMAIL: <a href='mailto:olivajonaj@gmail.com' class='project-link'>[ INICIATE MAIL TRANSMISSION ]</a>",
        github: "GITHUB: <a href='https://github.com/Jonathan-Oliva' target='_blank' class='project-link'>github.com/Jonathan-Oliva</a>",
        linkedin: "LINKEDIN: <a href='https://www.linkedin.com/in/jonathan-oliva-8a020b368/' target='_blank' class='project-link'>linkedin.com/in/jonathan-oliva-8a020b368/</a>",
        systemStandby: ">>> END OF CONNECTION. SYSTEM ON STANDBY_"
    },
    es: {
        skipBtn: "[ OMITIR ANIMACIÓN - ACCESO RÁPIDO ]",
        langBtn: "EN",
        intro: [
            "> INICIALIZANDO SISTEMA...",
            "> RESOLVIENDO DEPENDENCIAS...",
            "> CARGANDO INTERFAZ GRÁFICA..."
        ],
        accessGranted: ">>> ACCESO CONCEDIDO. CARGANDO PERFIL_DEL_SISTEMA...",
        personalDataTitle: "DATOS PERSONALES",
        name: "NOMBRE: JONATHAN OLIVA",
        role: "ROL: FULLSTACK DEVELOPER & TECH LEAD / CEO @ NIBBLE",
        location: "UBICACIÓN: Villa Mercedes, San Luis, Argentina",
        contactTitle: "CONTACTO",
        emailLink: "EMAIL: <a href='mailto:olivajonaj@gmail.com' class='project-link'>[ INICIAR TRANSMISIÓN DE CORREO ]</a>",
        github: "GITHUB: <a href='https://github.com/Jonathan-Oliva' target='_blank' class='project-link'>github.com/Jonathan-Oliva</a>",
        linkedin: "LINKEDIN: <a href='https://www.linkedin.com/in/jonathan-oliva-8a020b368/' target='_blank' class='project-link'>linkedin.com/in/jonathan-oliva-8a020b368/</a>",
        techTitle: "STACK TECNOLÓGICO Y ARQUITECTURA",
        techSubtitle: "> Lenguajes y herramientas que utilizo en producción:",
        backend: "<span class='skill-category'>BACKEND:</span> Python, Node.js, TypeScript, Java",
        frameworks: "<span class='skill-category'>FRAMEWORKS:</span> Django, Express.js",
        infra: "<span class='skill-category'>INFRA & DATOS:</span> Docker, PostgreSQL, SQL, APIs RESTful",
        product: "<span class='skill-category'>PRODUCTO (UI/UX):</span> Conceptualización de Producto, Canva, Experiencia de Devs",
        projectsTitle: "PROYECTOS DESTACADOS",
        project1: "1. GESTOR DE TURNOS DEPORTIVOS (Nibble)",
        project1Desc1: "> Arquitectura back-end escalable implementada en Python/Django y Dockerizada.",
        project1Desc2: "> Manejo robusto de concurrencia: implementación de bloqueos atómicos temporales para evitar solapamientos en reservas de canchas.",
        project1Demo: "> Demo Funcional: <a href='https://app.alapelotita.com.ar/' target='_blank' class='project-link'>app.alapelotita.com.ar</a>",
        project2: "2. SISTEMA DE RESERVAS PARA PELUQUERÍAS",
        project2Desc1: "> API REST construida con Node.js/Express para la autogestión de citas.",
        project2Desc2: "> Logica compleja de asignación de rangos horarios según la disponibilidad del negocio.",
        project2Demo: "> Demo Funcional: <a href='https://cirobarber.com.ar/' target='_blank' class='project-link'>cirobarber.com.ar</a>",
        project3: "3. NIBBLE STARTUP - PORTFOLIO CORPORATIVO",
        project3Desc1: "> Como CEO, liderando el diseño UI/UX, branding (logos, estética) y la implementación de la web productiva.",
        project3Demo: "> Demo: <a href='https://nibble-web-lemon.vercel.app/' target='_blank' class='project-link'>nibble-web-lemon.vercel.app</a>",
        project4: "4. SISTEMA DE VOTO ELECTRÓNICO (Auditoría / Académico)",
        project4Desc1: "> Desarrollo Backend con foco en seguridad e integridad de sufragios, mentorizado directamente por un Ingeniero Senior.",
        project4Desc2: "> Soft Skills en Producción: Representante técnico, presentador y defensor del sistema en medios de radio locales.",
        project4Repo: "> Repositorio: <a href='https://github.com/EnzoL1016/Proyecto-Unvime' target='_blank' class='project-link'>github.com/EnzoL1016/Proyecto-Unvime</a>",
        systemStandby: ">>> FIN DE LA CONEXIÓN. SISTEMA EN ESPERA_"
    }
};

let currentLang = 'en';
let skipAnimation = false;
let currentRenderId = 0;

const container = document.getElementById('text-container');
const skipBtn = document.getElementById('skip-btn');
const langBtn = document.getElementById('lang-btn');

// Inyectamos textos iniciales
skipBtn.textContent = i18n[currentLang].skipBtn;
langBtn.textContent = i18n[currentLang].langBtn;

const triggerSkip = (e) => {
    if (e && e.target === langBtn) return; // Evitar que cliquear lenguaje lo skipee como efecto secundario
    if (!skipAnimation) {
        skipAnimation = true;
        skipBtn.style.display = 'none';
    }
};

skipBtn.addEventListener('click', triggerSkip);
document.addEventListener('keydown', triggerSkip);
document.addEventListener('click', triggerSkip);

langBtn.addEventListener('click', async (e) => {
    e.stopPropagation(); // Evitar que se propague el click y dispare skip

    // Cambiar el idioma
    currentLang = currentLang === 'en' ? 'es' : 'en';

    // Actualizar los botones
    langBtn.textContent = i18n[currentLang].langBtn;
    skipBtn.textContent = i18n[currentLang].skipBtn;

    // Reiniciamos la animación en el nuevo idioma
    skipAnimation = false;
    skipBtn.style.display = 'block';
    container.innerHTML = ""; // Limpiamos la pantalla previamente

    await init();
});

async function typeLine(renderId, text, speed = 15, isTitle = false, className = "line") {
    // Si cambió el contexto de renderizado durante espera asíncrona, cortamos el hilo antiguo.
    if (renderId !== currentRenderId) return;

    const line = document.createElement('div');
    line.className = className;
    if (isTitle) line.classList.add('section-title');
    container.appendChild(line);

    if (skipAnimation) {
        line.innerHTML = text; // Bypass completo
        return;
    }

    if (text.includes('<a') || text.includes('<span')) {
        line.innerHTML = text;
        await new Promise(r => setTimeout(r, speed * 2));
    } else {
        for (let char of text) {
            if (renderId !== currentRenderId) return; // Chequeo constante de cierre de hilo asíncrono
            line.textContent += char;
            if (!skipAnimation) {
                window.scrollTo(0, document.documentElement.scrollHeight);
                await new Promise(r => setTimeout(r, speed));
            }
        }
        if (renderId !== currentRenderId) return;
        if (skipAnimation) line.innerHTML = text;
    }
}

async function showProfile() {
    currentRenderId++;
    const renderId = currentRenderId;
    container.innerHTML = "";
    if (skipAnimation) skipBtn.style.display = 'none';

    const text = i18n[currentLang];

    await typeLine(renderId, text.accessGranted, 10);

    await typeLine(renderId, text.personalDataTitle, 0, true);
    await typeLine(renderId, text.name);
    await typeLine(renderId, text.role);
    await typeLine(renderId, text.location);

    await typeLine(renderId, text.techTitle, 0, true);
    await typeLine(renderId, text.techSubtitle);
    await typeLine(renderId, text.backend, 5, false, "skill-item");
    await typeLine(renderId, text.frameworks, 5, false, "skill-item");
    await typeLine(renderId, text.infra, 5, false, "skill-item");
    await typeLine(renderId, text.product, 5, false, "skill-item");

    await typeLine(renderId, text.projectsTitle, 0, true);

    await typeLine(renderId, text.project1, 0, false, "project-title");
    await typeLine(renderId, text.project1Desc1);
    await typeLine(renderId, text.project1Desc2);
    await typeLine(renderId, text.project1Demo);
    await typeLine(renderId, "");

    await typeLine(renderId, text.project2, 0, false, "project-title");
    await typeLine(renderId, text.project2Desc1);
    await typeLine(renderId, text.project2Desc2);
    await typeLine(renderId, text.project2Demo);
    await typeLine(renderId, "");

    await typeLine(renderId, text.project3, 0, false, "project-title");
    await typeLine(renderId, text.project3Desc1);
    await typeLine(renderId, text.project3Demo);
    await typeLine(renderId, "");

    await typeLine(renderId, text.project4, 0, false, "project-title");
    await typeLine(renderId, text.project4Desc1);
    await typeLine(renderId, text.project4Desc2);
    await typeLine(renderId, text.project4Repo);
    await typeLine(renderId, "");

    await typeLine(renderId, text.contactTitle, 0, true);
    await typeLine(renderId, text.emailLink);
    await typeLine(renderId, text.github);
    await typeLine(renderId, text.linkedin);

    await typeLine(renderId, "");
    await typeLine(renderId, text.systemStandby);
}

async function init() {
    currentRenderId++;
    const renderId = currentRenderId;
    const text = i18n[currentLang];

    for (const msg of text.intro) {
        if (renderId !== currentRenderId) return;
        await typeLine(renderId, msg, 15);
        if (!skipAnimation) await new Promise(res => setTimeout(res, 200));
    }

    if (renderId === currentRenderId) {
        await showProfile();
    }
}

document.addEventListener('DOMContentLoaded', init);